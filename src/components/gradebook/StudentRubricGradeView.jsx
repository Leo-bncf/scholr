    ...
    <div className="space-y-6">
          ...
          <div className="flex items-start justify-between">
            ...
            <div className="text-right">
              {visibleComponents?.score && (
                <div>
                  <p className="text-sm scholr-muted">Score</p>
                  <p className="text-2xl font-bold scholr-ink">{totalScore}</p>
                </div>
              )}
            </div>
          </div>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-3 gap-4">
            {visibleComponents?.percentage && (
              <div>
                <p className="text-sm scholr-muted">Percentage</p>
            ...
            {visibleComponents?.ib_grade && showIBGrade && (
              <div>
                <p className="text-sm scholr-muted">IB Grade</p>
                <p className="text-xl font-bold scholr-accent">{gradeItem.ib_grade || '-'}</p>
              </div>
            )}
            <div>
              <p className="text-sm scholr-muted">Performance</p>
          ...
          {visibleComponents?.comment && gradeItem.comment && (
            <div className="pt-4 border-t">
              <p className="text-sm font-semibold scholr-body mb-2">Teacher Comment</p>
              <p className="text-sm scholr-muted">{gradeItem.comment}</p>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      {visibleComponents?.rubric_breakdown && (
        <div className="space-y-3">
          <h3 className="font-semibold scholr-ink">Assessment Criteria Breakdown</h3>
                  ...
                  <div className="space-y-3">
                    ...
                    {visibleComponents?.criteria_scores && criterionScore?.feedback && (
                      <div className="pt-2 border-t">
                        <p className="text-sm font-semibold scholr-body mb-1">Feedback</p>
                        <p className="text-sm scholr-muted">{criterionScore.feedback}</p>
                      </div>
                    )}
                  </div>
                </CardContent>
