...
/**
 * The standing state of the school. One number, one place.
 *
 * This used to carry an Activity panel showing the attendance rate and the
 * missing-work rate, and a Class coverage panel showing how many classes had a
 * teacher and how many had students. All four of those are also raised as
 * alerts directly above by OperationalAlerts, so a school admin read the same
 * four facts twice on one screen — once as a warning and once as a ratio —
 * and had to work out that they were the same thing.
 *
 * The rule now: a number appears once, in the instrument panel, wearing its
 * status. An alert is reserved for something that is not already a number on
 * this page and that has somewhere to go and fix it.
 */
export default function SchoolHealthOverview({ data }) {
  const {
    members, classes, attendanceRate, missingWorkRate, messagingVolume,
    upcomingTerms, setupDone, setupTotal,
  } = data;

  const pct = (v) => (v === null || v === undefined ? '—' : `${v}%`);

  // The thresholds the alert list used before these became figures, so a
  // school sees the same judgement in the same place rather than losing it.
  const attendanceTone = attendanceRate === null ? undefined
    : attendanceRate < 75 ? 'crit' : attendanceRate < 90 ? 'warn' : 'good';
  const missingTone = missingWorkRate === null ? undefined
    : missingWorkRate > 30 ? 'warn' : 'good';
    ...
    <div className="flex flex-col gap-5 md:gap-6">
      ...
      <StatRow>
        <StatCard
          ...
          hint={attendanceRate === null ? 'nothing recorded yet' : 'last 30 days'}
        />
        <StatCard
          label="Work missing"
          tone={missingTone}
          value={pct(missingWorkRate)}
          hint={missingWorkRate === null ? 'nothing set yet' : 'of expected submissions'}
        />
        <StatCard label="Messages" value={messagingVolume ?? 0} hint="rolling 30 days" />
        <StatCard
          label="Setup"
          value={`${setupDone ?? 0} / ${setupTotal ?? 5}`}
          hint={setupDone >= setupTotal ? 'complete' : 'steps done'}
        />
      </StatRow>
