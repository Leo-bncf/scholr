...
export default function WizardStepCurriculum({ schoolId, currentCurriculum, onDone }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h3 className="text-base font-bold scholr-ink mb-1">Choose Your Curriculum</h3>
            ...
            <button
              ...
              className={`text-left p-3.5 rounded-xl border-2 transition-colors ${
                isSelected
                  ...
                  : 'scholr-rule bg-white hover:scholr-rule'
              }`}
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  <p className={`text-sm font-semibold ${isSelected ? 'scholr-accent' : 'scholr-ink'}`}>{opt.label}</p>
                  <p className="text-xs scholr-muted mt-0.5">{opt.description}</p>
                </div>
                {isSelected && <CheckCircle2 className="w-4 h-4 scholr-accent shrink-0 mt-0.5" />}
              </div>
            </button>
          );
        })}
      ...
      {/* Template preview */}
      <div className="scholr-sunk border scholr-rule rounded-xl p-4 space-y-3">
        <div className="flex items-center gap-2">
          <GraduationCap className="w-4 h-4 scholr-accent" />
          <p className="text-xs font-semibold scholr-body uppercase tracking-wide">{config.shortLabel} — What gets pre-configured</p>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
          <div>
            <p className="scholr-faint mb-1">Grading Scale</p>
            <Badge variant="outline" className="text-xs">{config.gradeScale.displayLabel}</Badge>
          </div>
          <div>
            <p className="scholr-faint mb-1">Grade Levels</p>
            <div className="flex flex-wrap gap-1">
              {config.gradeLevels.length > 0
                ...
                : <span className="scholr-faint italic">Custom</span>
              }
              {config.gradeLevels.length > 4 && <span className="scholr-faint text-xs">+{config.gradeLevels.length - 4} more</span>}
            </div>
          </div>
          <div>
            <p className="scholr-faint mb-1">Coordinator Role</p>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {template.slice(0, 10).map(s => (
                <span key={s.code} className="text-xs app-group px-2 py-0.5 scholr-muted">{s.name}</span>
              ))}
              {template.length > 10 && (
                <span className="text-xs scholr-faint px-2 py-0.5">+{template.length - 10} more</span>
              )}
            </div>
          </div>
        )}
        {template.length === 0 && (
          <p className="text-xs scholr-faint italic">No subject template — you'll create subjects manually.</p>
        )}
      </div>
