...
export default function WorkspaceGradingPanel({ row, open, onClose, onSaveDraft, onPublishGrade }) {
  ...
  return (
    <div className={`fixed inset-y-0 right-0 z-50 w-full max-w-2xl bg-white border-l scholr-rule shadow-2xl transform transition-transform duration-200 ${open ? 'translate-x-0' : 'translate-x-full'}`}>
      <div className="h-full flex flex-col">
        <div className="px-5 py-4 border-b scholr-rule flex items-start justify-between gap-4 scholr-sunk">
          <div>
            <p className="text-lg font-bold scholr-ink">{row?.studentName || 'Submission'}</p>
            <p className="text-sm scholr-muted mt-1">{row?.assignmentName || 'Review and grade submission'}</p>
          </div>
          <button onClick={onClose} className="h-9 w-9 rounded-lg hover:scholr-sunk flex items-center justify-center scholr-muted">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div className="rounded-xl border scholr-rule p-4">
                <h3 className="text-sm font-bold scholr-ink mb-3">Submission viewer</h3>
                {row?.submission?.content ? (
                  <div className="rounded-lg scholr-sunk p-4 text-sm scholr-body whitespace-pre-wrap">{row.submission.content}</div>
                ) : (
                  <div className="rounded-lg scholr-sunk p-4 text-sm scholr-faint">No written response</div>
                )}
              </div>
            ...
            <div className="space-y-4">
              <div className="rounded-xl border scholr-rule p-4">
                <h3 className="text-sm font-bold scholr-ink mb-4">Rubric criteria</h3>
                <div className="space-y-4">
                  {criteria.map((criterion) => (
                    <div key={criterion.criterion_id} className="rounded-lg border scholr-rule p-3 space-y-3">
                      <div>
                        <p className="text-sm font-semibold scholr-ink">{criterion.name}</p>
                        {criterion.description && <p className="text-xs scholr-muted mt-1">{criterion.description}</p>}
                      </div>
                      <div>
                        <Label>Score</Label>
                        ...
                        <p className="text-[11px] scholr-faint mt-1">Out of {criterion.max_score}</p>
                      </div>
                      <div>
                        <Label>Comment</Label>
              ...
              <div className="rounded-xl border scholr-rule p-4 space-y-3">
                <div>
                ...
                <div className="text-sm font-semibold scholr-ink">Total score: {totalScore}</div>
              </div>
            </div>
          </div>
        </div>

        <div className="px-5 py-4 border-t scholr-rule bg-white flex items-center justify-end gap-3">
