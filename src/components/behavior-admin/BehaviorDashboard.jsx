...
const TYPE_META = {
  ...
  note:     { label: 'Note',     bg: 'scholr-sunk',   text: 'scholr-body',   border: 'scholr-rule',   icon: FileText },
};
...
function StatCard({ label, value, accent, icon: Icon }) {
  const a = {
    ...
    violet:  'scholr-accent-sf scholr-accent-rule scholr-accent',
...
export default function BehaviorDashboard({ schoolId, isPastoral = false }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Filters */}
      <div className="bg-white rounded-xl border scholr-rule p-4 flex flex-wrap gap-3 items-end">
        <div className="flex-1 min-w-44">
          <label className="text-xs font-semibold scholr-muted block mb-1">Search</label>
        ...
        <div>
          <label className="text-xs font-semibold scholr-muted block mb-1">Type</label>
        ...
        <div>
          <label className="text-xs font-semibold scholr-muted block mb-1">Severity</label>
          <select value={filterSeverity} onChange={e => setFilterSeverity(e.target.value)} className="px-3 py-1.5 border scholr-rule rounded-lg text-sm bg-white">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="scholr-sunk border-b scholr-rule">
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold scholr-muted uppercase">Title / Category</th>
                <th className="px-4 py-3 text-center text-xs font-semibold scholr-muted uppercase">Type</th>
                <th className="px-4 py-3 text-center text-xs font-semibold scholr-muted uppercase">Severity</th>
                <th className="px-4 py-3 text-center text-xs font-semibold scholr-muted uppercase">Visibility</th>
                <th className="px-4 py-3 text-left text-xs font-semibold scholr-muted uppercase">Recorded By</th>
                {isPastoral && <th className="px-4 py-3 text-center text-xs font-semibold scholr-muted uppercase">Reviewed</th>}
              </tr>
            </thead>
              ...
              {filtered.map(record => {
                ...
                return (
                  <React.Fragment key={record.id}>
                    <tr
                      className={`hover:scholr-sunk cursor-pointer ${record.staff_only ? 'bg-rose-50/30' : ''}`}
                      onClick={() => setExpandedId(expanded ? null : record.id)}
                    >
                      <td className="px-4 py-3 font-medium scholr-ink">{record.student_name}</td>
                      <td className="px-4 py-3 scholr-muted">{record.date}</td>
                      <td className="px-4 py-3">
                        <p className="font-medium scholr-ink truncate max-w-xs">{record.title}</p>
                        {record.category && <p className="text-xs scholr-faint capitalize mt-0.5">{record.category.replace(/_/g,' ')}</p>}
                      </td>
                      ...
                      <td className="px-4 py-3 text-center">
                        {sm ? <span className={`px-2 py-0.5 rounded-full text-xs font-medium border ${sm.bg} ${sm.text} ${sm.border}`}>{record.severity}</span> : <span className="scholr-faint">—</span>}
                      </td>
                      <td className="px-4 py-3 text-center">
                        {record.staff_only ? (
                          ...
                          <div className="flex items-center justify-center gap-1.5">
                            <span className={`text-xs ${record.visible_to_student ? 'text-blue-600' : 'scholr-faint'}`} title="Student"><Eye className="w-3.5 h-3.5" /></span>
                            <span className={`text-xs ${record.visible_to_parent ? 'text-emerald-600' : 'scholr-faint'}`} title="Parent"><Eye className="w-3.5 h-3.5" /></span>
                          </div>
                        )}
                      </td>
                      <td className="px-4 py-3 text-xs scholr-muted">{record.recorded_by_name}</td>
                      {isPastoral && (
                        <td className="px-4 py-3 text-center">
                    ...
                    {expanded && (
                      <tr>
                        <td colSpan={isPastoral ? 8 : 7} className="px-6 py-4 scholr-sunk border-b scholr-rule">
                          <div className="grid grid-cols-2 gap-4 text-sm">
                            {record.description && (
                              <div className="col-span-2">
                                <p className="text-xs font-bold scholr-muted mb-1 uppercase">Description</p>
                                <p className="scholr-body">{record.description}</p>
                              </div>
                            )}
                            {record.action_taken && (
                              <div>
                                <p className="text-xs font-bold scholr-muted mb-1 uppercase">Action Taken</p>
                                <p className="scholr-body">{record.action_taken}</p>
                              </div>
                            )}
                            {record.follow_up_required && (
                              <div>
                                <p className="text-xs font-bold scholr-muted mb-1 uppercase">Follow-up</p>
                                ...
                                {record.follow_up_note && <p className="text-xs scholr-muted mt-1">{record.follow_up_note}</p>}
                              </div>
                            )}
                            {record.pastoral_reviewed && (
                              <div>
                                <p className="text-xs font-bold scholr-muted mb-1 uppercase">Pastoral Review</p>
                                <p className="text-xs scholr-muted">Reviewed by {record.pastoral_reviewed_by} on {record.pastoral_reviewed_at ? format(new Date(record.pastoral_reviewed_at), 'dd MMM yyyy') : ''}</p>
                              </div>
                            )}
                          </div>
                        </td>
      ...
      <p className="text-xs scholr-faint text-right">{filtered.length} records shown</p>
    </div>
  );
}