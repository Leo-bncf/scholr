...
export default function TeacherTodayTab({ classData, onNavigate }) {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-5">
      <div>
        <h2 className="text-sm font-semibold scholr-ink">Today</h2>
        <p className="text-xs scholr-muted">What needs your attention in this class right now.</p>
      </div>
      ...
      <div className="bg-white rounded-xl border scholr-rule p-5">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h3 className="text-sm font-semibold scholr-ink">Coming up</h3>
            <p className="text-[11px] scholr-muted">Next assignments due in this class</p>
          </div>
          <Button variant="ghost" size="sm" className="h-7 text-xs scholr-muted" onClick={() => onNavigate('assignments')}>
            View all <ArrowRight className="w-3 h-3 ml-1" />
          </Button>
        </div>

        {upcoming.length === 0 ? (
          <div className="py-8 text-center text-xs scholr-faint">
            <Calendar className="w-6 h-6 mx-auto mb-2 scholr-faint" />
              ...
              return (
                <div key={a.id} className="flex items-center justify-between gap-3 py-2.5">
                  <div className="min-w-0">
                    <p className="text-sm font-medium scholr-ink truncate">{a.title}</p>
                    <p className="text-[11px] scholr-muted capitalize">{a.type?.replace('_', ' ') || 'assignment'}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className={`text-xs font-semibold ${overdue ? 'text-red-600' : isToday(a._due) ? 'text-amber-600' : 'scholr-muted'}`}>
                      {isToday(a._due) ? 'Today' : format(a._due, 'MMM d')}
                    </p>
                    <p className="text-[10px] scholr-faint">{format(a._due, 'h:mm a')}</p>
                  </div>
                </div>
              );
            })}
...
function ActionCard({ icon: Icon, title, status, primary, secondary, cta, onClick, disabled }) {
  const statusStyles = {
    ...
    info:    { bg: 'bg-white', border: 'scholr-rule', icon: 'scholr-accent', dot: 'bg-indigo-400' },
  }[status] || { bg: 'bg-white', border: 'scholr-rule', icon: 'scholr-muted', dot: 'scholr-sunk' };

  return (
    <div className={`rounded-xl border p-4 ${statusStyles.bg} ${statusStyles.border}`}>
      <div className="flex items-start justify-between mb-3">
        <div className={`w-9 h-9 rounded-lg bg-white border scholr-rule flex items-center justify-center ${statusStyles.icon}`}>
          <Icon className="w-4 h-4" />
        </div>
        <span className={`w-2 h-2 rounded-full ${statusStyles.dot}`} />
      </div>
      <p className="text-[11px] font-medium scholr-muted uppercase tracking-wide">{title}</p>
      <p className="text-lg font-semibold scholr-ink mt-0.5">{primary}</p>
      <p className="text-xs scholr-muted mt-0.5 truncate">{secondary}</p>
      <Button
        size="sm"
        variant="ghost"
        disabled={disabled}
        onClick={onClick}
        className="h-7 text-xs scholr-accent hover:scholr-accent hover:scholr-accent-sf -ml-2 mt-2"
      >
