...
const statusColors = {
  ...
  reviewed: 'scholr-accent-sf scholr-accent',
      ...
      <div className="min-h-screen scholr-sunk">
        ...
        <main className="app-offset p-8">
          <div className="max-w-7xl mx-auto">
            <div className="mb-8">
              <h1 className="text-3xl font-bold scholr-ink mb-2">IB Core: Theory of Knowledge</h1>
              <p className="scholr-muted">Track your TOK assignments, reflections, and projects</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
              <div className="bg-white rounded-xl border scholr-rule p-6">
                <p className="text-sm scholr-muted mb-1">Total Tasks</p>
                <p className="text-3xl font-bold scholr-ink">{tasks.length}</p>
              </div>
              <div className="bg-white rounded-xl border scholr-rule p-6">
                <p className="text-sm scholr-muted mb-1">Pending</p>
                <p className="text-3xl font-bold text-amber-600">{tasks.filter(t => t.status === 'pending').length}</p>
              </div>
              <div className="bg-white rounded-xl border scholr-rule p-6">
                <p className="text-sm scholr-muted mb-1">Completed</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white rounded-xl border scholr-rule p-6">
                <h2 className="text-xl font-bold scholr-ink mb-4">Upcoming Tasks</h2>
                {isLoading ? (
                    ...
                    {upcomingTasks.map(task => (
                      <div key={task.id} className="border scholr-rule rounded-lg p-4">
                        <div className="flex items-start justify-between mb-2">
                          <h3 className="font-semibold scholr-ink">{task.title}</h3>
                          <Badge variant="outline" className="capitalize">{taskTypeLabels[task.task_type]}</Badge>
                        </div>
                        {task.description && (
                          <p className="text-sm scholr-muted mb-3 line-clamp-2">{task.description}</p>
                        )}
                        <div className="flex items-center justify-between">
                          <span className="text-xs scholr-muted flex items-center gap-1">
              ...
              <div className="bg-white rounded-xl border scholr-rule p-6">
                <h2 className="text-xl font-bold scholr-ink mb-4">All Tasks</h2>
                {isLoading ? (
                  <div className="flex justify-center py-12">
                    <Loader2 className="w-6 h-6 animate-spin scholr-accent" />
                  </div>
                  ...
                  <div className="space-y-3 max-h-96 overflow-y-auto">
                    {tasks.map(task => (
                      <div key={task.id} className="border scholr-rule rounded-lg p-4">
                        <div className="flex items-start justify-between mb-2">
                          <h3 className="font-medium scholr-ink text-sm">{task.title}</h3>
                        ...
                        {task.teacher_feedback && task.status === 'reviewed' && (
                          <div className="mt-2 scholr-sunk rounded p-2">
                            <p className="text-xs scholr-muted">{task.teacher_feedback}</p>
                          </div>
                        )}
                      </div>
                    ))}
