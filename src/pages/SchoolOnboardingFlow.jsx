...
import { Loader2, ArrowLeft, ArrowRight, Save, Sparkles, CheckCircle2 } from 'lucide-react';
...
const steps = [
  { id: 'school_profile', label: 'Create school profile' },
  { id: 'classes', label: 'Add classes' },
  { id: 'teachers', label: 'Add teachers' },
  { id: 'students', label: 'Add students' },
  { id: 'subjects', label: 'Assign subjects' },
      ...
      <div className="min-h-screen bg-slate-50">
        ...
        <main className="md:ml-64 min-h-screen p-6 max-w-7xl mx-auto space-y-6">
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div>
              <h1 className="text-2xl font-bold text-slate-900">School Onboarding Flow</h1>
              <p className="text-sm text-slate-500 mt-1">Set up your school quickly and finish with a ready admin dashboard.</p>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => navigate('/SchoolAdminDashboard')}><ArrowLeft className="w-4 h-4 mr-2" />Exit</Button>
              <Button variant="outline" onClick={saveProgress} disabled={saving}>{saving ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Save className="w-4 h-4 mr-2" />}Save progress</Button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[320px_minmax(0,1fr)] gap-6">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-6">
              <div>
                ...
                <h2 className="text-xl font-bold text-slate-900 mt-2">{steps[currentStep].label}</h2>
              </div>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-slate-200">
                <Button variant="outline" onClick={handleBack} disabled={currentStep === 0}><ArrowLeft className="w-4 h-4 mr-2" />Back</Button>
                <Button onClick={handleNext}>{currentStep === steps.length - 1 ? 'Finish setup' : 'Next'}<ArrowRight className="w-4 h-4 ml-2" /></Button>
              </div>
            </div>
          </div>
        </main>
